...
// akkor a kiírást megelõzi egy BELL karakter kiküldése
...
static void init_serialport (char *device, int speed) {
	...
	cfsetispeed(&options, getspeed(speed)); // bejövõ baud beállítása
	cfsetospeed(&options, getspeed(speed)); // kimenõ baud beállítása
	options.c_cflag |= (CLOCAL | CREAD); // vevõ engedélyezése, lokális mód beállítása
